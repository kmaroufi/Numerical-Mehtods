 ...
 */
public class Chapter6SelectingMethodsScreen implements Screen {
    ...
    public Chapter6SelectingMethodsScreen(Screen parent) {
        this.parent = parent;
    }
...
//        table.setBackground(new TextureRegionDrawable(new TextureRegion(new Texture(Gdx.files.internal("background4.jpg")))));
        ...
        table.row();
        //method label
        table.add().height(Util.multiplyToWorldFactor(200)).colspan(7);
        Cell methodsLabel = table.add().height(Util.multiplyToWorldFactor(200)).colspan(10).right();
        {
            Label.LabelStyle labelStyle = new Label.LabelStyle(font, Color.BLACK);
            Label label = new Label("Choose your method :  ", labelStyle);
            label.setFontScale(Util.multiplyToWorldFactor(.7f));
            methodsLabel.setActor(label);
        }
        Cell methodsCell =  table.add().height(Util.multiplyToWorldFactor(70)).colspan(8).left();
        {
...
//            scrollPaneStyle.hScroll = new TextureRegionDrawable(new TextureRegion(new Texture("hScroll.png")));
            ...
            selectBox.setItems(new String[]{"Inverse matrix", "Cramer", "Gausse Elimination", "LU Doolittle", "LU Cholesky",
                "LU Crout", "Jacobi", "Gauss Seidel"});
            methodsCell.setActor(selectBox);
        }
        ...
        {
            Label.LabelStyle labelStyle = new Label.LabelStyle(font, Color.BLACK);
            Label label = new Label("Enter number of equations :  ", labelStyle);
            label.setFontScale(Util.multiplyToWorldFactor(.7f));
    ...
    private void openRelatedScreen() {
        ...
        String function = (String) selectBox.getSelected();
        Integer n = Integer.valueOf(nTextField.getText());
        switch (index) {
            case 0:
            case 1:
            case 2:
            ...
            case 4:
            case 5:
                ((Game)Gdx.app.getApplicationListener()).setScreen(new Chapter6Methods1Screen(Chapter6SelectingMethodsScreen.this, function, n));
                break;
            case 6:
            case 7:
                ((Game)Gdx.app.getApplicationListener()).setScreen(new Chapter6Methods2Screen(Chapter6SelectingMethodsScreen.this, function, n));
        }
    }
