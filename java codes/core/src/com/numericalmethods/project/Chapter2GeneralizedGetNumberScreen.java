 ...
 */
public class Chapter2GeneralizedGetNumberScreen implements Screen{
    ...
    public Chapter2GeneralizedGetNumberScreen(Screen parent, String method) {
        this.parent = parent;
        this.method = method;
    }
...
//        table.setBackground(new TextureRegionDrawable(new TextureRegion(new Texture(Gdx.files.internal("background4.jpg")))));
        ...
        {
            Label.LabelStyle labelStyle = new Label.LabelStyle(font, Color.BLACK);
            Label label = new Label("Enter number of variables :  ", labelStyle);
        ...
        Cell numberOfVariablesTextCell =  table.add().height(Util.multiplyToWorldFactor(70)).colspan(4).fillX();
        {
            ...
            numberOfVariablesTextField = new TextField("", textFieldStyle);
        ...
        Cell generateFields = table.add().height(Util.multiplyToWorldFactor(100)).colspan(26);
        {
            TextButton.TextButtonStyle textButtonStyle = new TextButton.TextButtonStyle();
            textButtonStyle.font  = font;
            textButtonStyle.fontColor = Color.BLACK;
            ...
            textButtonStyle.up = new NinePatchDrawable(new NinePatch(new Texture(Gdx.files.internal("button/up.png")), 35,35,35,35));
            textButtonStyle.over = textButtonStyle.down;
            TextButton textButton = new TextButton("Generate fields for variables and functions", textButtonStyle);
            textButton.getLabel().setFontScale(.8f * Util.multiplyToWorldFactor(1));
            textButton.addListener(new ClickListener() {
                @Override
                public void clicked(InputEvent event, float x, float y) {
                    ((Game)Gdx.app.getApplicationListener()).setScreen(
                            new Chapter2GeneralizedGetInputsScreen(Chapter2GeneralizedGetNumberScreen.this,  Integer.valueOf(numberOfVariablesTextField.getText())));
                }
            });
