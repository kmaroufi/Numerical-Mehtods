...
/**
 * Created by asus-pc on 12/8/2016.
 */
public class Chapter3CurveFittingScreen implements Screen{
    ...
    public Chapter3CurveFittingScreen(Screen parent, String function, double[] pointx, double[] pointy) {
        this.parent = parent;
...
//        table.setBackground(new TextureRegionDrawable(new TextureRegion(new Texture(Gdx.files.internal("background4.jpg")))));
        ...
        table.row();
        //method label
        table.add().height(Util.multiplyToWorldFactor(200)).colspan(15);
        Cell choose = table.add().height(Util.multiplyToWorldFactor(200)).colspan(15).right();
        {
            Label.LabelStyle labelStyle = new Label.LabelStyle(font, Color.BLACK);
            Label label = new Label("Choose your function to be fitted :  ", labelStyle);
            label.setFontScale(Util.multiplyToWorldFactor(.7f));
            choose.setActor(label);
        }
        Cell choosingCell =  table.add().height(Util.multiplyToWorldFactor(70)).colspan(6);
        {
...
//            selectBox.setItems(new String[]{"nothing","a*e^(b*x)", "a*ln(x)+b", "a/x+b","1/(a*x+b)","Sai*x^i(i <= 3)"});
            selectBox.setItems(new String[]{"nothing","a*x^2+b*x+c", "1/a*x+b", "a*log(x)+b","a/x+b","a*exp(b*x)", "a*sin(x)+b",
            "a*cos(x)+b","a*tan(x)+b","a*asin(x)+b","a*acos(x)+b","a*atan(x)+b","a*sinh(x)+b","a*cosh(x)+b","tanh(x)","asinh(x)",
            "acosh(x)","atanh(x)"});
            choosingCell.setActor(selectBox);
        }
        table.add().height(Util.multiplyToWorldFactor(200)).colspan(14);
        ...
        table.row();
        // optional point
        table.add().height(Util.multiplyToWorldFactor(100)).colspan(12);
        ...
        Cell calculating = table.add().height(Util.multiplyToWorldFactor(100)).colspan(10);
        {
            ...
            textButtonStyle.up = new NinePatchDrawable(new NinePatch(new Texture(Gdx.files.internal("button/up.png")), 35,35,35,35));
            textButtonStyle.over = textButtonStyle.down;
            calculatingButton = new TextButton("Curve fitting", textButtonStyle);
    ...
    public void calculating() {
        ...
        String fittedFunction = (String) selectBox.getSelected();
        // "Lagrange", "Newton Divided Differences", "Newton Forward, Backward and Central Differences"
        ...
        String input8 = fittedFunction;
        if (fittedFunction.equals("nothing")) {
            input8 = "";
        }
        MatlabEngine engine = ((Project) Gdx.app.getApplicationListener()).getEngine();
        try {
            String methodName = "Curve_Fitting";
//            if (fittedFunction.equals("Lagrange")) {
//                methodName = "Lagrange";
//            } else if (fittedFunction.equals("Newton Divided Differences")) {
//                methodName = "falsePosition";
//            } else if (fittedFunction.equals("Newton Forward, Backward and Central Differences")) {
//                methodName =  "secant";
//            }
            Object[] object = engine.feval(2,  methodName, input1,input2,input3, input4, input5, input6, input7, input8);
