...
/**
 * Created by asus-pc on 1/3/2017.
 */
public class Chapter5FirstOrderScreen implements Screen {
    ...
    public Chapter5FirstOrderScreen(Screen parent) {
        this.parent = parent;
        degree = "First-Order ODE";
    }
...
//            scrollPaneStyle.hScroll = new TextureRegionDrawable(new TextureRegion(new Texture("hScroll.png")));
            ...
            selectBox.setItems(new String[]{"Taylor", "Euler",
                    "Runge-Kutta Mid-Point", "Runge-Kutta 3rd Order","Runge-Kutta 4th Order", "Adams-Moulton"});
...
//        symbol = table.add().height(Util.multiplyToWorldFactor(100)).colspan(1);
//        {
//            Label.LabelStyle labelStyle = new Label.LabelStyle(font, Color.BLACK);
//            Label label = new Label(" , ", labelStyle);
//            label.setFontScale(Util.multiplyToWorldFactor(.5f));
...
//        }
        ...
        Cell nLabel = table.add().height(Util.multiplyToWorldFactor(100)).colspan(5).right();
        {
            Label.LabelStyle labelStyle = new Label.LabelStyle(font, Color.BLACK);
            Label label = new Label("Enter your n :  ", labelStyle);
            label.setFontScale(Util.multiplyToWorldFactor(.5f));
            nLabel.setActor(label);
        }
        Cell nTextCell = table.add().height(Util.multiplyToWorldFactor(70)).colspan(6).fillX();
        {
            TextField.TextFieldStyle textFieldStyle = new TextField.TextFieldStyle();
            textFieldStyle.font = font;
            textFieldStyle.messageFont = font;
    ...
    public void calculating() {
        ...
        double input6 = Double.parseDouble(String.valueOf(selectBox.getSelectedIndex() + 1));
        double input7 = 0;
        if (!nTextField.getText().equals("")) {
            input7 = Double.parseDouble(nTextField.getText());
        }
        ...
        try {
            Object[] result = engine.feval(2,  "calculator", input1,input2,input3,input4,input5,input6,input7,input8);
            String[] answerY = (String[]) result[0];
            String[] answerX = (String[]) result[1];
            showResult(answerY, answerX);
    ...
    private void showResult(String[] answerY, String[] answerX) {
...
//        viewport.setScreenBounds(0, 0 , Gdx.graphics.getWidth(), Gdx.graphics.getHeight());
        ...
        if (selectBox.getSelectedIndex() != 0) {
            nTextField.setText("");
            nTextField.setDisabled(true);
            nTextField.getStyle().background = textureRegionDrawable2;

        } else {
            nTextField.getStyle().background = textureRegionDrawable1;
            nTextField.setDisabled(false);
        }
